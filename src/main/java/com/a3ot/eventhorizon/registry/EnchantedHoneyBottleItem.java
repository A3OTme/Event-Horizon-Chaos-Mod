...
public class EnchantedHoneyBottleItem extends HoneyBottleItem {
    ...
    private static void sendEventMessage(ServerLevel level, AbstractEvent event) {
        level.getServer().getPlayerList().getPlayers().forEach(player -> {
            player.sendSystemMessage(
                    Component.literal("[Event Horizon] ").append(Component.translatable("item.eventhorizon.enchanted_honey_bottle.clear",
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
        tooltip.add(Component.translatable("item.eventhorizon.enchanted_honey_bottle.description")
